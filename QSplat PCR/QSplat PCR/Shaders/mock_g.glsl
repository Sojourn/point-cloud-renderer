...
void SetVertex(vec4 position, vec3 normal, vec4 color, float alpha) {
...
void main() {
	...
	for (int m = 0; m < num_triangles; m++) {
		SetVertex(gl_in[0].gl_Position, normal, gColor[0],0);
		
		vec4 rVector = vec4(gRadius[0] * (cos(angle) * i + sin(angle) * j), 0);
		SetVertex(gl_in[0].gl_Position + rVector, normal, gColor[0],1);
